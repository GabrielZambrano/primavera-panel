...
function TaxiForm() {
  ...
  const [mapaVisible, setMapaVisible] = useState(false); // Para controlar el mapa desde el formulario
   ...
   const [pedidosCombinados, setPedidosCombinados] = useState([]);
  ...
  // Cargar pedidos combinados de ambas colecciones
  const cargarPedidosCombinados = async () => {
    setCargandoPedidos(true);
    try {
      // Cargar pedidos disponibles
      const qDisponibles = query(collection(db, 'pedidosDisponibles'));
      const snapshotDisponibles = await getDocs(qDisponibles);
      const pedidosDisponibles = snapshotDisponibles.docs.map(doc => ({
        id: doc.id,
        ...doc.data(),
        tipo: 'Disponible'
      }));
      ...
      const snapshotEnCurso = await getDocs(qEnCurso);
      const pedidosEnCurso = snapshotEnCurso.docs.map(doc => ({
        id: doc.id,
        ...doc.data(),
        tipo: 'En Curso'
      }));

      // Combinar ambas listas por ID
      const pedidosCombinados = [];
      const idsUnicos = new Set([
        ...pedidosDisponibles.map(p => p.id),
        ...pedidosEnCurso.map(p => p.id)
      ]);

      idsUnicos.forEach(id => {
        const disponible = pedidosDisponibles.find(p => p.id === id);
        const enCurso = pedidosEnCurso.find(p => p.id === id);
        
        // Crear un objeto combinado con datos de ambas colecciones
        const pedidoCombinado = {
          id: id,
          disponible: disponible || null,
          enCurso: enCurso || null,
          estado: enCurso ? 'En Curso' : 'Disponible',
          fechaCreacion: disponible?.fecha || enCurso?.fecha || '',
          nombreCliente: disponible?.nombreCliente || enCurso?.nombreCliente || '',
          telefono: disponible?.telefono || enCurso?.telefono || '',
          direccion: disponible?.direccion || enCurso?.direccion || '',
          destino: disponible?.destino || enCurso?.destino || '',
          clave: disponible?.clave || enCurso?.clave || '',
          idConductor: enCurso?.idConductor || '',
          nombreConductor: enCurso?.nombre || '',
          placa: enCurso?.placa || '',
          distancia: enCurso?.distancia || '',
          minutos: enCurso?.minutos || ''
        };
        
        pedidosCombinados.push(pedidoCombinado);
      });
      ...
      setPedidosCombinados(pedidosCombinados);
    } catch (error) {
      console.error('Error al cargar pedidos combinados:', error);
    } finally {
  ...
  const handleAddressSelect = useCallback((nuevaDireccion) => {
    setDireccion(nuevaDireccion);
     ...
     <div style={{
      ...
      {/* Tabla de Pedidos Disponibles y En Curso */}
      <div style={{
        marginTop: 40,
        background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)',
        borderRadius: 16,
        ...
        <div style={{
          ...
          <h3 style={{
            ...
            <span style={{
              ...
              {pedidosCombinados.length} registros
            </span>
          </h3>
          <button
            onClick={cargarPedidosCombinados}
            ...
            style={{
              ...
              padding: '8px 16px',
              borderRadius: 8,
              cursor: cargandoPedidos ? 'not-allowed' : 'pointer',
              fontSize: 14,
              fontWeight: 'bold',
              opacity: cargandoPedidos ? 0.7 : 1
            }}
          >
        ...
        {cargandoPedidos ? (
          ...
          <div style={{
            ...
            <div style={{ fontSize: 14 }}>
              Los pedidos aparecerán aquí cuando estén disponibles o en curso
            </div>
          </div>
        ) : (
          <div style={{ overflowX: 'auto' }}>
            <table style={{
              ...
              <tbody>
                {pedidosCombinados.map((pedido, index) => (
                  <tr
                    key={pedido.id}
                    style={{
                      borderBottom: '1px solid #f1f5f9',
                  ...
                  >
                    <td style={{
                      ...
                      {pedido.fechaCreacion ? 
                        new Date(pedido.fechaCreacion).toLocaleDateString('es-EC') + ' ' +
                        new Date(pedido.fechaCreacion).toLocaleTimeString('es-EC', {
                          hour: '2-digit',
                          minute: '2-digit'
                        })
                        : '-'}
                    ...
                    <td style={{
                      ...
                      <button
                        style={{
                          ...
                          fontWeight: 'bold',
                          cursor: 'default',
                          background: pedido.estado === 'En Curso' ? '#dc2626' : '#10b981',
                          color: 'white'
                        }}
                      >
                        {pedido.estado}
